"""
Lambda Function: fetch_station_active_raw

서울시 공공자전거 실시간 대여정보(bikeList / rentBikeStatus)를 수집하여 S3 raw 영역에 적재합니다.
과거 데이터 조회가 불가능한 API이므로 매일 00:10 KST에 EventBridge에 의해 트리거되어
당일 스냅샷을 안전하게 보존합니다.

착지 경로:
    s3://<RAW_BUCKET>/raw/station_active/api/snapshot_date=YYYY-MM-DD/payload.json
"""
import json
import logging
import os
...
def _extract_service_node(body: Dict[str, Any], service: str) -> Dict[str, Any]:
    ...
    # bikeList의 실제 root key는 'rentBikeStatus'
    if "rentBikeStatus" in body:
        return body["rentBikeStatus"]
    if service in body:
        return body[service]
...
def fetch_all_station_active(
    ...
    """bikeList API를 페이징 순회하여 전체 실시간 대여소 상태를 반환합니다."""
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    logger.info("Starting raw fetch: service=bikeList, snapshot_date=%s, bucket=%s", snapshot_date, raw_bucket)
    rows = fetch_all_station_active(api_key=api_key, base_url=base_url)
    logger.info("Fetched %d rows for station_active", len(rows))
