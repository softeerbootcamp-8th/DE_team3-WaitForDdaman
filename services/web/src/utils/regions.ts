...
// region이 없어서, 지도 위 구 강조표시(구 이름만 아는 경우)에 쓸 매핑을 대여소 목록에서 파생시킨다.
export function buildDistrictSideMap(stations: MapStation[]): Record<string, Side> {
...
export function isDistrictActive(district: string, filter: RegionFilter, districtToSide: Record<string, Side>): boolean {
  if (filter.kind === "all") return false;
  if (filter.kind === "side") return districtToSide[district] === filter.side;
  return district === filter.name;
}
