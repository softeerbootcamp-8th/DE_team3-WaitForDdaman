...
export function DistrictMap({
  ...
  const riskStations = useMemo(() => stations?.filter((s) => s.riskCount > 0) ?? [], [stations]);
  const maxRisk = useMemo(() => Math.max(1, ...riskStations.map((s) => s.riskCount)), [riskStations]);
    ...
    <div className={`map-card${variant === "mini" ? " mini" : ""}`} ref={mapCardRef} style={{ aspectRatio: `${w} / ${h}` }}>
      ...
      {hover && (
        <div className="station-tooltip show" style={{ left: hover.left, top: hover.top }}>
          <b>{hover.station.name}</b>
          <br />
          위험 자전거 {hover.station.riskCount}대 / 전체 {hover.station.bikeCount}대 (거치대 {hover.station.holdNum}대) ·
