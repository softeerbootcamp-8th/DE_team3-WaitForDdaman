...
export function DistrictMap({
  ...
  const riskStations = useMemo(() => stations?.filter((s) => s.riskCnt > 0) ?? [], [stations]);
  const maxRisk = useMemo(() => Math.max(1, ...riskStations.map((s) => s.riskCnt)), [riskStations]);
  ...
  return (
    <div className={`map-card${variant === "mini" ? " mini" : ""}`} ref={mapCardRef} style={{ aspectRatio: `${w} / ${h}` }}>
      <svg viewBox={`0 0 ${w} ${h}`}>
        ...
        {showStationDots &&
          riskStations.map((s) => {
            const r = 1.8 + 3.2 * Math.sqrt(s.riskCnt / maxRisk);
            return (
              <circle
                key={s.stationId}
      ...
      {hover && (
        <div className="station-tooltip show" style={{ left: hover.left, top: hover.top }}>
          <b>{hover.station.stationName}</b>
          <br />
          위험 자전거 {hover.station.riskCnt}대 / 전체 {hover.station.bikeCnt}대 (거치대 {hover.station.holdNum}대) ·
          정상비율 {hover.station.healthyRatio}% ({hover.station.urgency})
        </div>
      )}
    </div>
  );
