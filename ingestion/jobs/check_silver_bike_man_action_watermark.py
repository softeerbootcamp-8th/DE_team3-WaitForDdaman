"""
Gold DAG(dag_gold_dim_fact)의 wait_for_silver 단계 - bike_man_action 준비 확인용

### 왜 ExternalTaskSensor를 안 쓰는가
silver_bike_man_action_daily DAG는 고정 cron이 아니라 Asset(bikeman_event_bronze)
...
def run() -> None:
    ...
    watermark = read_watermark(watermark_key=SILVER_BIKE_MAN_ACTION)

    if watermark >= target_date:
        logger.info("silver.bike_man_action 준비 완료 (워터마크=%s >= 대상일=%s)", watermark, target_date)
        sys.exit(0)

    logger.info("silver.bike_man_action 아직 준비 안 됨 (워터마크=%s < 대상일=%s)", watermark, target_date)
    sys.exit(1)
