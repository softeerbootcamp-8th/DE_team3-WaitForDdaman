...
def _write_all_fresh(as_of: date, offset_days: int = 1):
    ...
    from jobs.check_watermark_staleness import WATERMARK_DATASETS
...
def test_all_fresh_watermarks_are_not_stale(s3_env):
    from jobs.check_watermark_staleness import stale_datasets
...
def test_missing_watermark_counts_as_stale(s3_env):
    ...
    from jobs.check_watermark_staleness import stale_datasets, WATERMARK_DATASETS
...
def test_run_passes_silently_when_fresh(s3_env):
    from jobs.check_watermark_staleness import run
...
def test_run_raises_when_stale(s3_env, monkeypatch):
    from jobs.check_watermark_staleness import run, WatermarkStalenessError
